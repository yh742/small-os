...
static unsigned char kbd_read_scan_code();
static char kbd_scan_code_to_ascii(unsigned char);

void kbd_init()
{
	register_interrupt_handler(KBD_INT_IDX, &kbd_handle);
}

void kbd_handle(struct cpu_info state, struct idt_info info, struct stack_state exec)
{
	UNUSED(state);
	UNUSED(info);
	UNUSED(exec);

	unsigned char scan_code = kbd_read_scan_code();
	char c = kbd_scan_code_to_ascii(scan_code);
...
static char kbd_scan_code_to_ascii(unsigned char scan_code)
{
    char ch = -1;
